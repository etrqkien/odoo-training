...
class ChiTietDatHang(models.Model):
    _name = 'ct.dat.hang'
    _description = 'Trang mô tả chi tiết đơn hàng'
    ...
    nguoi_thanh_toan = fields.Many2one('res.partner', string='Người thanh toán')
    da_thanh_toan = fields.Boolean(string='Đã thanh toán')
    payment = fields.Selection(selection=[('cash', 'Tiền mặt'), ('bank', 'Thanh toán chuyển khoản'), ('ewallet', 'Ví điện tử')],string='Hình thức thanh toán')
    ...
    def _compute_devide_type(self):
        for rec in self:
            if rec.order.type_discount == 'percent':
